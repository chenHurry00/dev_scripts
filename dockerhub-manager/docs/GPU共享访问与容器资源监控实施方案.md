...
```text
容器本体镜像快照 + 原有挂载/volume/资源配置一起参与重建
```
...
重建时如果选择“先按当前状态生成镜像再重建”，则这次生成的镜像属于：

```text
temporary_rebuild_snapshot
```
...
| 类型 | 来源 | 是否自动删除 |
| --- | --- | --- |
| `temporary_rebuild_snapshot` | 重建流程临时生成 | 是 |
| `manual_backup` | 用户手动备份 | 否 |
